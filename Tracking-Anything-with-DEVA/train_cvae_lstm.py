...
class AllDataDataset(Dataset):
    def __init__(self, data_path, config, feature_extractor=None, lstm_seq_len=20):
        self.data_path = data_path
        self.config = config
        self.feature_extractor = feature_extractor
        self.use_frozen_features = feature_extractor is not None
        self.lstm_seq_len = lstm_seq_len
        self.use_lstm = feature_extractor is not None and feature_extractor.output_type == "lstm"
        ...
        # For LSTM: store episode-wise data and extract features
        if self.use_lstm:
            self.episodes = []
            ...
            for idx, fname in enumerate(files):
                ...
                try:
                    ...
                    T = len(frames)
                    # Need at least lstm_seq_len+1 frames for LSTM
                    if T < self.lstm_seq_len + 1:
                    ...
                    episode_data = {
                        ...
                        'target_id': self.target_ids[target],
                        'linear_v_id': self.linear_v_ids[linear_v],
                        'angular_v_id': self.angular_v_ids[angular_v],
            ...
            print(f"episodes: {len(self.episodes)}, trackers: {sorted(self.tracker_ids.keys())}, targets: {sorted(self.target_ids.keys())}, linear V: {sorted(self.linear_v_ids.keys())}, angular V: {sorted(self.angular_v_ids.keys())}")
            
            # Extract LSTM features for all valid sequences (similar to CNN feature extraction)
            print(f"\n Extracting LSTM features from episodes...")
            all_state_features = []
            all_next_state_features = []
            all_actions = []
            all_rewards = []
            all_tracker_ids = []
            all_target_ids = []
            ...
            total_sequences = sum(ep['length'] - self.lstm_seq_len for ep in self.episodes)
            print(f"   Total sequences to extract: {total_sequences}")
            processed = 0
            
            for ep_idx, episode in enumerate(self.episodes):
                if ep_idx % 50 == 0:
                    print(f"   Processing episode {ep_idx}/{len(self.episodes)}")
                
                # Extract all valid sequences from this episode
                for start_idx in range(episode['length'] - self.lstm_seq_len):
                    # Extract current sequence
                    curr_seq = []
                    for i in range(self.lstm_seq_len):
                        frame = episode['frames'][start_idx + i]
                        mask = frame['mask'][:, :, 0:3]
                    ...
                    next_seq = []
                    for i in range(self.lstm_seq_len):
                        frame = episode['frames'][start_idx + i + 1]
                        mask = frame['mask'][:, :, 0:3]
                        resized = cv2.resize(mask, (64, 64))
                        chw = np.transpose(resized, (2, 0, 1)).astype(np.float32)
                        next_seq.append(chw)
                    ...
                    curr_seq_tensor = torch.from_numpy(np.array(curr_seq)).float().unsqueeze(0).to(self.feature_extractor.device)
                    next_seq_tensor = torch.from_numpy(np.array(next_seq)).float().unsqueeze(0).to(self.feature_extractor.device)
                    
                    with torch.no_grad():
                        state_feat, _ = self.feature_extractor(curr_seq_tensor)
                        next_state_feat, _ = self.feature_extractor(next_seq_tensor)
                        state_feat = state_feat.squeeze(0).cpu().numpy()
                        next_state_feat = next_state_feat.squeeze(0).cpu().numpy()
                    
                    all_state_features.append(state_feat)
                    all_next_state_features.append(next_state_feat)
                    
                    # Get action and reward from last frame of sequence
                    last_frame_idx = start_idx + self.lstm_seq_len - 1
                    action = np.array(episode['frames'][last_frame_idx]['action']).squeeze().astype(np.float32)
                    all_actions.append(action)
                    
                    state_img = np.array(episode['frames'][last_frame_idx]['mask'][:, :, 0:3])
                    goal_img = np.array(episode['frames'][last_frame_idx]['goal'][:, :, 0:3])
                    reward_val = reward_cal(state_img, goal_img)
                    all_rewards.append(np.array([reward_val], dtype=np.float32))
                    
                    all_tracker_ids.append(episode['tracker_id'])
                    all_target_ids.append(episode['target_id'])
                    all_linear_v_ids.append(episode['linear_v_id'])
                    all_angular_v_ids.append(episode['angular_v_id'])
                    all_linear_v_maxs.append(episode['linear_v_max'])
                    all_angular_v_maxs.append(episode['angular_v_max'])
                    
                    processed += 1
                    if processed % 1000 == 0:
                        print(f"      Extracted {processed}/{total_sequences} sequences")
            
            # Convert to numpy arrays (same as CNN mode)
            self.state_features = np.array(all_state_features, dtype=np.float32)
            self.next_state_features = np.array(all_next_state_features, dtype=np.float32)
            ...
            print(f"✅ LSTM feature extraction completed!")
            print(f"   State features shape: {self.state_features.shape}")
            print(f"   Total samples: {len(self.state_features)}")
            
        else:
            # Original non-LSTM loading
            all_states = []
            all_actions = []
            all_rewards = []
            all_next_states = []
    ...
    def __getitem__(self, idx):
        if self.use_lstm:
            # LSTM mode: return pre-extracted features (same as CNN mode)
            return (
...
def get_config():
    ...
    parser.add_argument("--run_name", type=str, default="CVAE-mutidim-lstm-", help="run_name")
    ...
    parser.add_argument("--use_lstm_features", action="store_true", default=True)
...
def main():
    ...
    if config.frozen_cnn_lstm_path and (config.use_cnn_features or config.use_lstm_features):
        if config.use_lstm_features:
            output_type = "lstm"
        else:
            output_type = "cnn"
    ...
    train_dataset = AllDataDataset(config.data_path, config, feature_extractor=feature_extractor, lstm_seq_len=config.lstm_seq_len)
